...
\begin{comment}
Este capítulo trata da definição da proposta deste trabalho, dos procedimentos executados na entrevista para diagnosticar a situação do desenvolvimento descentralizado no Órgão X, das descrições das fontes complementares para a elaboração da solução: o Processo de Desenvolvimento Descentralizado existente e a \textit{Wiki} da unidade coordenadora do desenvolvimento descentralizado, e da metodologia de construção da solução de apoio.
\end{comment}

\begin{comment}
Este trabalho se propõe a elaborar uma solução informatizada que possa apoiar o desenvolvimento descentralizado no Órgão X. Esta solução irá conter um conjunto de atividades, de boas práticas e de funcionalidades que possam contribuir para uma melhoria na qualidade e manutenibilidade das aplicações desenvolvidas. Na primeira etapa do trabalho foi identificado o conjunto de atividades e boas práticas da solução, de forma que estas atividades estivessem relacionadas entre si através de um fluxograma, e que as boas práticas estivessem contidas dentro das atividades, auxiliando a execução das mesmas. Na segunda etapa realizou-se a implementação do modelo da solução gerada.
\end{comment}
...
\begin{comment}
A construção deste trabalho foi guiada pelas as etapas de pesquisa previamente definidas. Dentre estas etapas, temos:

\begin{itemize}
\item Definir o problema objeto de estudo, os objetivos a serem alcançados e a escolha da metodologia;
\item Estudos bibliográficos de forma a embasar o trabalho;
\item Planejar e executar a entrevista semi-estruturada com os indivíduos alvos, previamente selecionados;
\item Análise dos documentos do órgão e das entrevistas;
\item Construção do modelo da solução de apoio;
\item Implementação do modelo da solução de apoio;
\end{itemize}
\end{comment}

\section{Caracterização do Órgão}

O presente trabalho está inserido no contexto do órgão público de estudo, o qual possui duas formas distintas de desenvolvimento de software: uma forma corporativa, que é uma forma tradicional de desenvolvimento de software (desenvolvimento realizado na área da TI e por profissionais de TI), e uma forma descentralizada, que é uma forma menos tradicional de desenvolvimento (desenvolvimento realizado nas áreas de negócio e por pessoas com menos qualificação técnica e experiência, geralmente estagiários).

O desenvolvimento descentralizado no órgão público de estudo segue a linha do EUD e, devido a isso, a forma de desenvolvimento é bastante flexível, variando conforme o contexto da unidade de negócio e a experiência do desenvolvedor. A ferramenta adotada pelo órgão para viabilizar o desenvolvimento descentralizado foi o Oracle APEX, que é bastante amigável e permite que pessoas com muito pouca experiência em programação possam desenvolver aplicações. Percebendo a variação na forma de desenvolvimento dos sistemas, a unidade coordenadora do desenvolvimento descentralizado propôs:\clearpage
...
perguntas sobre os
parâmetros do projeto                       & Não                  \\ \cline{3-4} 
                                                                    &                      & H02 - Visualizar guia de técnica de elicitação (Fácil)             & Sim                  \\ \cline{3-4}
 &  &  H03 - Cadastrar técnicas de elicitação & Não \\ \cline{3-4}
 &  & H04 - Editar técnicas de elicitação & Não \\ \cline{3-4}
 &  & H05 - Listar técnicas de elicitação & Não \\ \cline{3-4}
 &  & H06 - Cadastrar parâmetros do projeto referente as técnicas & Não \\ \cline{3-4}                                                       
 &  & H07 - Editar parâmetros do projeto referente as técnicas & Não \\ \cline{3-4}
 &  & H08 - Listar parâmetros do projeto referente as técnicas & Não \\ \cline{3-4}
 &  & H09 - Cadastrar pesos dos parâmetros referente as técnicas & Não \\ \cline{3-4}
 &  & H10 - Editar pesos dos parâmetros referente as técnicas & Não \\ \cline{3-4}
...
&  & H05 - Cadastrar informações levantadas na elicitação de requisitos & Não \\ \cline{3-4}                                
&  & H06 - Editar informações levantadas na elicitação de requisitos & Não \\ \cline{3-4}
&  & H07 - Listar informações levantadas na elicitação de requisitos & Não \\ \cline{3-4}
&  & H08 - Excluir informações levantadas na elicitação de requisitos & Não \\ \cline{3-4}
&  & H09 - Cadastrar atores no sistema a ser desenvolvido & Não \\ \cline{3-4}
&  & H10 - Editar atores no sistema a ser desenvolvido & Não \\ \cline{3-4}
&  & H11 - Listar atores no sistema a ser desenvolvido & Não \\ \cline{3-4}
&  & H12 - Excluir atores no sistema a ser desenvolvido & Não \\ \cline{3-4}
&  & H13 - Cadastrar \textit{features} do projeto & Não \\ \cline{3-4}
&  & H14 - Editar \textit{features} do projeto & Não \\ \cline{3-4}
&  & H15 - Listar \textit{features} do projeto & Não \\ \cline{3-4}
&  & H16 - Excluir \textit{features} do projeto & Não \\ \cline{3-4}
&  & H17 - Cadastrar funcionalidades do projeto relacionado a \textit{feature} & Não \\ \cline{3-4}
&  & H18 - Editar funcionalidades do projeto relacionado a \textit{feature} & Não \\ \cline{3-4}
&  & H19 - Listar funcionalidades do projeto relacionado a \textit{feature} & Não \\ \cline{3-4}
&  & H20 - Excluir funcionalidades do projeto relacionado a uma \textit{feature} & Não \\ \cline{3-4}
&  & H21 - Cadastrar requisitos não-funcionais do projeto & Não \\ \cline{3-4}
&  & H22 - Editar requisitos não-funcionais do projeto & Não \\ \cline{3-4}
&  & H23 - Listar requisitos não-funcionais do projeto & Não \\ \cline{3-4}
&  & H24 - Excluir requisitos não-funcionais do projeto & Não \\ \cline{3-4}
&  & H25 - Cadastrar regras de negócio do projeto & Não \\ \cline{3-4}
&  & H26 - Editar regras de negócio do projeto & Não \\ \cline{3-4}
&  & H27 - Listar regras de negócio do projeto & Não \\ \cline{3-4}
&  & H28 - Excluir regras de negócio do projeto & Não \\ \cline{3-4}
...
F03 - Construir modelo de dados                    & \multirow{7}{*}{Sim} & H01 - Guia de criação e padrões de BD (Médio)                     & Sim                  \\ \cline{3-4} 
                                                                    ...
                                                                    &                      & H05 - Validar modelo de dados (Médio)                            & Sim                  \\ \cline{3-4} 
                                                                    &                      & H06 - Atualizar modelo de dados (Fácil)                           & Sim                  \\ \cline{3-4} 
                                                                    &                      & H07 - Cadastrar modelo de dados e informações referentes & Não                  \\ \cline{3-4}
 &  & H08 - Excluir modelo de dados & Não \\ \cline{3-4}
...
F08 - Implantação	& \multirow{2}{*}{Sim} & F08/H01 - Guia de Migração de Aplicações & Sim \\ \cline{3-4}
  &  & F08/H02 - Migrar Aplicação & Sim \\ \cline{3-4}
  &  & F08/H03 - Definir Procedimentos de Implantação & Sim \\ \hline
F09 Projeto Kanban (Gerência de projetos) & \multirow{2}{*}{Sim} & F09/H01 - Cadastrar projeto & Não \\ \cline{3-4}
 &  & F09/H02 - Editar projeto & Não \\ \cline{3-4}
...
\\ \cline{2-3}
 & F02 - Elicitar Requisitos Junto Ao Cliente & F02/H03 - Guia de prototipagem rápido (Fácil) & 
\\ \cline{2-3}
...
\\ \cline{3-3}
 &  & F03/H03 - Guia de geração de scripts sql (Fácil, Filipe) & 
\\ \cline{3-3}
 &  & F03/H05 - Validar modelo de dados (Médio, Fagner) & \multirow{1}{5.2cm}{F05/H04 - Listar código reutilizável (Fácil, Filipe)}
\\ \cline{2-3}
...
\\ \cline{2-4} 
 & \multirow{9}{3.0cm}{F01 - Escolher Técnica de elicitação de requisitos} & F01/H03 - Cadastrar técnicas de elicitação & F01/H03 - Cadastrar técnicas de elicitação
\\ \cline{3-4}
 &  & F01/H04 - Editar técnicas de elicitação & F01/H04 - Editar técnicas de elicitação
\\ \cline{3-4}
 &  & F01/H05 - Listar técnicas de elicitação & F01/H05 - Listar técnicas de elicitação
\\ \cline{3-4}
 &  & F01/H06 - Cadastrar parâmetros do projeto referente as técnicas & F01/H06 - Cadastrar parâmetros do projeto referente as técnicas
\\ \cline{3-4}
 &  & F01/H07 - Editar parâmetros do projeto referente as técnicas & F01/H07 - Editar parâmetros do projeto referente as técnicas
\\ \cline{3-4}
 &  & F01/H08 - Listar parâmetros do projeto referente as técnicas & F01/H08 - Listar parâmetros do projeto referente as técnicas
\\ \cline{3-4}
 &  & F01/H09 - Cadastrar pesos dos parâmetros referente as técnicas & F01/H09 - Cadastrar pesos dos parâmetros referente as técnicas
\\ \cline{3-4}
 &  & F01/H10 - Editar pesos dos parâmetros referente as técnicas & F01/H10 - Editar pesos dos parâmetros referente as técnicas
\\ \cline{3-4}
 &  & F01/H11 - Listar pesos dos parâmetros referente as técnicas & F01/H11 - Listar pesos dos parâmetros referente as técnicas
\\ \cline{2-4}
...
\\ \cline{3-4}
 &  & F03/H05 - Validar modelo de dados (Médio, Fagner) & \multirow{3}{5.2cm}{F04/H01 - Guia de padrões e boas práticas Apex (Médio, Filipe)}
\\ \cline{2-3}
...
\\ \cline{2-4} 
 & \multirow{16}{3.0cm}{F02 - Elicitar requisitos junto ao cliente} & F02/H05 - Cadastrar informações levantadas na elicitação de requisitos &  
...
\\ \cline{2-4}
 & F03 - Construir modelo de dados & F03/H06 - Atualizar modelo de dados (Fácil) & F03/H06 - Atualizar modelo de dados (Fácil)
\\ \cline{2-4}
 & F03 - Construir modelo de dados & F03/H07 - Cadastrar modelo de dados e informações referentes & F03/H07 - Cadastrar modelo de dados e informações referentes
\\ \cline{2-4}
 & F03 - Construir modelo de dados & F03/H08 - Excluir modelo de dados & F03/H08 - Excluir modelo de dados
\\ \cline{2-3}
...
Neste capítulo são apresentados as atividades e guias selecionados, resultantes tanto do diagnóstico obtido com as entrevistas, quanto do processo de desenvolvimento descentralizado e da \textit{Wiki}. Em seguida a solução gerada é apresentada. Por fim é apresentada a solução de apoio implementada.
...
Uma matriz de rastreabilidade das chaves foi elaborada, de forma a garantir a associação entre as chaves e as respostas, dadas pelos desenvolvedores, para cada pergunta. O conjunto de respostas das perguntas de 1 a 5, por se tratar meramente de uma contextualização do perfil dos entrevistados, não fez parte do escopo da análise. A tabela 18 ilustra a rastreabilidade de chaves com as respostas das perguntas.\clearpage
...
\caption{Atividades e guias resultantes da solução}\\\hline
...
Apesar da conformidade geral ser de 64 \%, houveram implementações finalizadas que não estavam relacionadas ao diagnóstico obtido, ou seja, essas implementações foram levantadas com base na observação de pesquisa durante a construção da solução (descritas na tabela 4).
