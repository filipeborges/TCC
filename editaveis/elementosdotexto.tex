...
\textbf{Feature}                                                    & \textbf{Origem no Diagnóstico} & \textbf{Estória}                                           & \textbf{Origem no Diagnóstico} \\ \hline
\endhead
F01 - Escolher Técnica de elicitação de requisitos & \multirow{2}{*}{Não} & H01 - Indicar técnica de elicitação (Médio)                       & Sim                  \\ \cline{3-4} 
                                                                    &                      & H02 - Visualizar guia de técnica de elicitação (Fácil)             & Sim                  \\ \hline
F02 - Elicitar Requisitos Junto Ao Cliente         & \multirow{4}{*}{Não} & H01 - Estruturar informações obtidas (Médio)                       & Não                  \\ \cline{3-4}
                                                                    &                      & H02 - Priorizar funcionalidades a serem desenvolvidas (Fácil)     & Não                  \\ \cline{3-4} 
                                                                    &                      & H03 - Guia de prototipagem rápido (Fácil)                          & Não                  \\ \cline{3-4} 
                                                                    &                      & H04 - Elicitar Requisitos Junto Ao Cliente (Fácil)                 & Não                  \\ \hline
F03 - Construir modelo de dados                    & \multirow{7}{*}{Não} & H01 - Guia de criação e padrões de BD (Médio)                     & Não                  \\ \cline{3-4} 
                                                                    &                      & H02 - Guia de padrão de nomenclatura de objetos de dados (Fácil)  & Não                  \\ \cline{3-4} 
                                                                    &                      & H03 - Guia de geração de scripts sql (Fácil)                      & Não                  \\ \cline{3-4} 
                                                                    &                      & H04 - Construir modelo de dados (Fácil)                          & Não                  \\ \cline{3-4} 
                                                                    &                      & H05 - Validar modelo de dados (Médio)                            & Não                  \\ \cline{3-4} 
                                                                    &                      & H06 - Atualizar modelo de dados (Fácil)                           & Não                  \\ \cline{3-4} 
                                                                    &                      & H07 - Validação de modelo de dados por responsável técnico (Médio) & Sim                  \\ \hline
F04 - Implementação da aplicação                   & \multirow{3}{*}{Sim} & H01 - Guia de padrões e boas práticas Apex (Médio)                 & Não                  \\ \cline{3-4} 
                                                                    &                      & H02 - Ajustar interface (Fácil)                                   & Não                  \\ \cline{3-4} 
                                                                    &                      & H03 - Aplicar regras de negócio (Fácil)                           & Não                  \\ \hline
F05 - Elaboração de código                         & \multirow{5}{*}{Sim} & H01 - Guia de boas práticas plsql (Fácil)                         & Não                  \\ \cline{3-4} 
                                                                    &                      & H02 - Guia sobre editor de código sql e plsql (Fácil)              & Não                  \\ \cline{3-4} 
                                                                    &                      & H03 - Cadastro de código reutilizável (Fácil)                      & Não                  \\ \cline{3-4} 
                                                                    &                      & H04 - Listar código reutilizável (Fácil)                          & Sim                  \\ \cline{3-4} 
                                                                    &                      & H05 - Editar código reutilizável (Fácil)                          & Sim                  \\ \hline
F06 - Procedimentos de teste                       & Sim & H01 - Preparar ambiente de testes (Fácil)                         & Não                  \\ \cline{3-4} 
                                                                    &                      & H02 - Definir procedimento de testes (Fácil)                       & Não                  \\ \cline{3-4} 
                                                                    &                      & H03 - Geração de casos de testes (Difícil)                          & Não                  \\ \cline{3-4} 
                                                                    &                      & H04 - Guia de ferramenta de automação de teste (Fácil)            & Não                  \\ \cline{3-4} 
                                                                    &                      & H05 - Guia de teste de comportamento de código (Fácil)            & Não                  \\ \cline{3-4} 
                                                                    &                      & H06 - Guia de teste de página apex (Fácil)                        & Não                  \\ \hline
F07 - Tratamento de erros                          & \multirow{2}{*}{Sim} & H01 - Guia de uso de depuração de páginas apex (Fácil)            & Não                  \\ \cline{3-4} 
...
A \textit{Sprint 1} teve duração de 1 semana e 2 dias, pois se tratava de uma \textit{Sprint} de ajuste. Foi concluída somente uma única estória de dificuldade média, e a estória F02/H03 não possuiu nenhum membro responsável pois estava como um adicional caso algum membro finalizasse suas estórias muito rapidamente. \clearpage
...
\begin{tabular}{|M{1.8cm}|M{3.0cm}|M{5.2cm}|M{5.2cm}|}
\hline
\multirow{6}{1.8cm}{\textbf{Sprint 2}} & \textbf{\textit{Features} Envolvidas} &\textbf{Planejado} & \textbf{Concluído} 
\\ \cline{2-4} 
 & \multirow{3}{3.0cm}{F03 - Construir modelo de dados} & F03/H02 - Guia de padrão de nomenclatura de objetos de dados (Fácil, Fagner) & \multirow{2}{5.2cm}{F03/H03 - Guia de geração de scripts sql (Fácil, Filipe)}
\\ \cline{3-3}
 &  & F03/H03 - Guia de geração de scripts sql (Fácil, Filipe) & 
\\ \cline{3-3}
 &  & F03/H05 - Validar modelo de dados (Médio, Fagner) & \multirow{2}{5.2cm}{F05/H03 - Cadastro de código reutilizável (Fácil, Filipe)}
\\ \cline{2-3}
 & \multirow{2}{3.2cm}{F05 - Elaboração de código} & F05/H03 - Cadastro de código reutilizável (Fácil, Filipe) & 
\\ \cline{3-3}
 &  & F05/H04 - Listar código reutilizável (Fácil, Filipe) & F05/H04 - Listar código reutilizável (Fácil, Filipe)
\\ \hline
\end{tabular}
\end{table}

Na \textit{Sprint} 2 as \textit{Features} 01 e 02 foram retiradas para a inclusão das \textit{Features} 03 e 05, consideradas de maior prioridade. Portanto as estórias F01/H01, F01/H02 e F02/H03 foram retiradas em favor da inclusão das estórias F03/H02, F03/H05, F05/H03 e F05/H04. 
...
\\ \cline{2-4} 
 & \multirow{2}{3.0cm}{F03 - Construir modelo de dados} & F03/H02 - Guia de padrão de nomenclatura de objetos de dados (Fácil, Fagner) & F05/H05 - Editar código reutilizável (Fácil, Filipe)
\\ \cline{3-3}
...
\\ \cline{2-4} 
 & F01 - Escolher Técnica de elicitação de requisitos & F01/H01 - Indicar técnica de elicitação (Difícil, Fagner) &  \multirow{3}{5.2cm}{F03/H02 - Guia de padrão de nomenclatura de objetos de dados (Fácil, Fagner)}
\\ \cline{3-3}
 &  & F01/H02 - Visualizar guia de técnica de elicitação (Fácil, Fagner) & 
\\ \cline{2-3}
...
\\ \cline{2-4} 
 & F01 - Escolher Técnica de elicitação de requisitos & F01/H01 - Indicar técnica de elicitação (Difícil, Fagner) &  \multirow{4}{5.2cm}{F07/H01 - Guia de uso de depuração de páginas apex (Fácil, Filipe)}
\\ \cline{3-3}
 &  & F01/H02 - Visualizar guia de técnica de elicitação (Fácil, Fagner) & 
\\ \cline{2-3}
...
\\ \cline{2-4} 
 & F01 - Escolher Técnica de elicitação de requisitos & F01/H01 - Indicar técnica de elicitação (Difícil, Fagner) &  
\multirow{3}{5.2cm}{F06/H03 - Geração de casos de testes (Difícil, Filipe)} 
\\ \cline{3-3}
 &  & F01/H02 - Visualizar guia de técnica de elicitação (Fácil, Fagner) & 
\\ \cline{2-3}
 & F06 - Procedimentos de teste & F06/H03 - Geração de casos de testes (Difícil, Filipe) & 
...
\begin{tabular}{|M{1.8cm}|M{3.0cm}|M{5.2cm}|M{5.2cm}|}
\hline
\multirow{4}{1.8cm}{\textbf{Sprint 10}} & \textbf{\textit{Features} Envolvidas} &\textbf{Planejado} & \textbf{Concluído} 
\\ \cline{2-4} 
...
\section{SISAEUD - Sistema de Apoio ao Desenvolvimento Descentralizado}

O SISAEUD, Sistema de Apoio ao Desenvolvimento Descentralizado, é a implementação da solução gerada, onde espera-se que contribua com o desenvolvimento descentralizado dentro do Órgão X. Este sistema foi desenvolvido na plataforma Oracle Application Express. As figuras de 14 a 19 mostram as capturas das telas mais representativas do SISAEUD.

\begin{figure}[!h]
	\hspace*{-1.5cm} 
		\includegraphics[scale=0.40]{figuras/home_p1}
	\caption{Tela inicial do SISAEUD}
\end{figure}

Na tela inicial do SISAEUD é apresentado o diagrama do ciclo iterativo proposto para o desenvolvimento descentralizado, onde cada etapa deste ciclo é um botão clicável que o desenvolvedor pode usar para acessar o conjunto de atividades, guias e funcionalidades da respectiva etapa. Além disso, o menu superior oferece uma navegação mais rápida e direta para as funcionalidades da aplicação. Observa-se que este menu superior é apresentado em todas as telas do sistema.
...
A tela de cadastro de informações de um projeto é uma das mais importantes do sistema, pois permite ao desenvolvedor cadastrar e editar informações a cerca do projeto no qual está trabalhando. A parte superior exibe várias abas onde o desenvolvedor pode alternar entre várias telas de cadastro/edição de informações sobre o projeto, como: Informações do Sistema, Atores do Sistema, \textit{Features}, Funcionalidades, Requisitos Não Funcionais, Regras de Negócio e Visualizar a Rastreabilidade dos Requisitos.
...
A tela de geração de valores para teste do tipo caixa preta gera um conjunto de valores para teste, seguindo o método BVA (\textit{Boundary Value Analysis}), de acordo com o componente selecionado. Os componentes apresentados para escolha são componentes de entrada de dados disponíveis na versão 5 do APEX (versão mais atual). Desta forma, permite-se que o processo de teste do tipo caixa preta das páginas desenvolvidas seja mais sistemático.
...
A tela de guia de padrão de nomenclatura de objetos de dados apresenta informações ao desenvolvedor de como nomear os objetos do banco de dados, de acordo com os padrões definidos pelo Órgão X. Em todas as telas de guias são apresentados o índice, de forma a facilitar a navegação, seguido do conteúdo das seções. \clearpage
