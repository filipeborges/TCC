...
O problema identificado para este trabalho se refere a maneira de como é executado o desenvolvimento de sistemas descentralizados no Órgão X. Uma vez que os sistemas são desenvolvidos de forma descentralizada, ou seja, alguns departamentos realizam o desenvolvimento de seus sistemas conforme suas necessidades ou solicitam o desenvolvimento a algum departamento especializado em desenvolvimento e suporte de sistemas, eles geralmente acabam ficando sem um padrão comum de desenvolvimento e sem o uso de boas práticas. \textbf{\textit{-- TODO: Verificar aqui problema que algum autor retrata que ocorre no órgão--}}
...
É importante que o órgão se esforce para melhorar a maneira de como são desenvolvidos seus sistemas descentralizados, uma vez que isso pode acarretar em problemas relacionados a qualidade e a manutenibilidade desses sistemas. Segundo \textit{Blackwell}, no caso do uso de ferramentas de programação é muito fácil para os desenvolvedores de software profissionais assumir que todas as pessoas irão se aproximar ao seu nível profissional de programação. É necessário atenção e investigação especializada para os usuários finais, para evitar essa armadilha quando os profissionais de software criarem novas tecnologias voltadas ao EUD. Baseado nesses argumentos, é necessário que os desenvolvedores EUD tenham uma atenção especial para o desenvolvimento de seus sistemas, com um modelo que lhes dê suporte.

\section{\textit{Metodologia}}

Para a escolha da estratégia de pesquisa, basicamente depende-se do atendimento de três condições: o tipo de questão de pesquisa, o controle que o pesquisador possui sobre os eventos comportamentais efetivos, e o foco em fenômenos históricos, em oposição a fenômenos contemporâneos \cite{yin2001estudo}. 
