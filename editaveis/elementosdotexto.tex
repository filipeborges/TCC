...
Para se atingir o objetivo deste trabalho, realizou-se, inicialmente, um estudo sobre os tópicos relacionados ao trabalho: \textit{End User Development}, desenvolvimento descentralizado, \textit{SCRUM} e \textit{Kanban}. Em seguida foi elaborado uma entrevista com o propósito de diagnosticar a situação do desenvolvimento descentralizado no Órgão X, para coletar os problemas, as boas práticas e as sugestões de melhorias, relatados pelos próprios desenvolvedores. Foi feito um estudo de um processo de desenvolvimento descentralizado definido no Órgão X, que nunca chegou a ser implantado, de forma a aproveitar algumas atividades e guias existentes nele, para a elaboração das soluções relacionadas ao diagnóstico obtido. Um estudo sobre os conteúdos da \textit{Wiki} da unidade coordenadora do desenvolvimento descentralizado também foi feito, para a agregação de algumas boas práticas, definidas pela unidade, à solução. A partir do diagnóstico, do estudo do processo e do estudo da \textit{Wiki}, foram definidas as atividades e as boas práticas da solução de apoio, e em seguida um relacionamento destas atividades foi feito, através da elaboração de um fluxograma.
...
Uma vez que a entrevista foi elaborada, o foco voltou-se a análise dos dados da mesma. A análise feita consistiu dos seguintes passos:
...
Foram selecionados, para a construção da solução de apoio, os componentes de processo (atividades, tarefas, guias e modelos) que possuem uma relação com os problemas, as sugestões e as boas práticas, relatados no diagnóstico.

\section{\textit{Wiki} do Desenvolvimento Descentralizado}

O Órgão X utiliza um sistema de \textit{Wiki} como uma das formas de compartilhamento de informação entre o público interno do órgão, de forma que esse público possa usufruir e contribuir com as informações publicadas (REFERENCIAR). A unidade coordenadora do desenvolvimento descentralizado também possui a sua página dentro da \textit{Wiki}, onde são definidos guias e boas práticas que contribuam para a melhoria no desenvolvimento descentralizado.
