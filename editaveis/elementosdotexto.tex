...
Afim de aspectos de aplicação e análise desse modelo proposto de solução, a próxima etapa deste  trabalho será a sua implementação utilizando a ferramenta de desenvolvimento comum ao órgão x, onde serão realizados em cada fase descrita, uma decomposição das atividades, guias e funcionalidades visando uma maior granularidade no que diz respeito aos requisitos necessários para respectivas implementações. De acordo com o cronograma, a cada fase depois de implementado a solução, será feito o teste em um projeto piloto e colhido os resultados para análise de atendimento ou não da solução ao que foi proposto.
