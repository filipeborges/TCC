...
A representação da seleção dos itens da metodologia adotada é representado na Figura \ref{metodologia_adotada}, onde são exibidos todas as opções disponíveis dentro de cada classificação, sendo as opções destacadas em preto as selecionadas para a execução do trabalho.
...
Para elucidar melhor as subfases da etapa operação da ação, foi elaborado uma estratégia para a implementação da solução do modelo elaborado na fase anterior, planejamento da ação, sendo essa adoção a adaptação das fases da pesquisa participante conforme a Figura \ref{figura_pesquisa_participante}. %\clearpage
